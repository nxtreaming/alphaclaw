...
const createAgentsService = ({
  ...
  const createChannelAccount = async (input = {}) => {
    ...
    if (duplicateEnvEntry) {
      throw new Error(`Channel token already exists in ${String(duplicateEnvEntry.key || "").trim()}`);
    }
    ...
    if (provider === "discord") {
      ...
      if (!String(nextProviderConfig.defaultAccount || "").trim()) {
      ...
      ensurePluginAllowed({ cfg: nextCfg, pluginKey: provider });
      const binding = appendBindingToConfig({
        cfg: nextCfg,
        agentId,
