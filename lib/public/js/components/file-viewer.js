...
export const FileViewer = ({
  ...
  const isLockedFile =
    canEditFile &&
    matchesBrowsePolicyPath(kLockedBrowsePaths, normalizedPolicyPath);
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      ...
      if (!canEditFile || isPreviewOnly || isDiffView || viewMode !== "edit")
        return;
                  ...
                  <div class="file-viewer-image-shell">
                    ${imageDataUrl
                      ? html`
                          <img
                            src=${imageDataUrl}
                            alt=${pathSegments[pathSegments.length - 1] ||
                            "Selected image"}
