...
export const AgentPairingSection = ({ agent = {} }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "alphaclaw:agent-bindings-changed",
      handleBindingsChanged,
    );
    return () => {
  ...
  const ownedAccounts = useMemo(() => {
    ...
    for (const binding of bindings) {
      ...
      const accountId =
        String(binding?.match?.accountId || "").trim() || "default";
  ...
  const boundChannels = useMemo(
    () =>
      Array.from(new Set(ownedAccounts.map((entry) => entry.channel))).filter(
        Boolean,
      ),
    [ownedAccounts],
  );
